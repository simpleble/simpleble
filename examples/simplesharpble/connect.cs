...
try
{
    Console.WriteLine($"Connected. MTU: {peripheral.Mtu}");
    foreach (var service in peripheral.Services)
    {
        ...
        foreach (var c in service.Characteristics)
        {
            ...
            Console.WriteLine($"    Read: {c.CanRead}, Write request: {c.CanWriteRequest}, " +
                $"Write command: {c.CanWriteCommand}, Notify: {c.CanNotify}, Indicate: {c.CanIndicate}");
            foreach (var descriptor in c.Descriptors)
