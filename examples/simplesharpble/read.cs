...
try
{
    var target = SelectCharacteristic(peripheral, c => c.CanRead);
    if (target is null) return;
    for (int i = 0; i < 5; i++)
    {
        byte[] value = peripheral.Read(target.Value.Service, target.Value.Characteristic);
        Console.WriteLine($"Read: {Convert.ToHexString(value)}");
