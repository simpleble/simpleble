...
try
{
    var target = SelectCharacteristic(peripheral, c => c.CanWriteRequest || c.CanWriteCommand);
    if (target is null) return;
    Console.Write("Write mode (request/command): ");
    string? mode = Console.ReadLine();
    if (mode != "request" && mode != "command") return;
    var selected = peripheral.Services.Single(s => s.Uuid == target.Value.Service)
        .Characteristics.Single(c => c.Uuid == target.Value.Characteristic);
    if ((mode == "request" && !selected.CanWriteRequest) || (mode == "command" && !selected.CanWriteCommand))
    {
        Console.WriteLine("The characteristic does not support that write mode.");
        return;
    }
    Console.Write("Hex bytes to write: ");
    string? input = Console.ReadLine();
    if (input is null) return;
    byte[] bytes;
    try { bytes = Convert.FromHexString(input.Replace(" ", "")); }
    catch (FormatException) { Console.WriteLine("Invalid hex bytes."); return; }
    if (mode == "request")
        peripheral.WriteRequest(target.Value.Service, target.Value.Characteristic, bytes);
    else
        peripheral.WriteCommand(target.Value.Service, target.Value.Characteristic, bytes);
}
finally
{
