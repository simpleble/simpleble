...
class HybridPeripheral : public HybridPeripheralSpec {
  public:
    ...
    std::shared_ptr<Promise<std::shared_ptr<ArrayBuffer>>> read(const std::string& service,
                                                                const std::string& characteristic) override;
    std::shared_ptr<Promise<void>> write_request(const std::string& service, const std::string& characteristic,
                                                 const std::shared_ptr<ArrayBuffer>& data) override;
    void write_command(const std::string& service, const std::string& characteristic,
                       const std::shared_ptr<ArrayBuffer>& data) override;
    std::shared_ptr<Promise<void>> notify(
    ...
    std::shared_ptr<Promise<void>> indicate(
        const std::string& service, const std::string& characteristic,
        const std::function<void(const std::shared_ptr<ArrayBuffer>&)>& callback) override;
    std::shared_ptr<Promise<void>> unsubscribe(const std::string& service, const std::string& characteristic) override;

    std::shared_ptr<Promise<std::shared_ptr<ArrayBuffer>>> read_descriptor(const std::string& service,
  ...
  private:
    ...
    std::map<std::pair<std::string, std::string>, std::function<void(const std::shared_ptr<ArrayBuffer>&)>>
        _notifyCallbacks;
