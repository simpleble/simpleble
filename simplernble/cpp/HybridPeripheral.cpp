...
bool HybridPeripheral::initialized() { return _peripheral.initialized(); }

std::string HybridPeripheral::identifier() { return _peripheral.identifier(); }

std::string HybridPeripheral::address() { return _peripheral.address(); }
...
double HybridPeripheral::rssi() { return static_cast<double>(_peripheral.rssi()); }

double HybridPeripheral::tx_power() { return static_cast<double>(_peripheral.tx_power()); }

double HybridPeripheral::mtu() { return static_cast<double>(_peripheral.mtu()); }

std::shared_ptr<Promise<void>> HybridPeripheral::connect() {
    return Promise<void>::async([this]() { _peripheral.connect(); });
}

std::shared_ptr<Promise<void>> HybridPeripheral::disconnect() {
    return Promise<void>::async([this]() { _peripheral.disconnect(); });
}

bool HybridPeripheral::is_connected() { return _peripheral.is_connected(); }

bool HybridPeripheral::is_connectable() { return _peripheral.is_connectable(); }

bool HybridPeripheral::is_paired() { return _peripheral.is_paired(); }

void HybridPeripheral::unpair() { _peripheral.unpair(); }
...
std::shared_ptr<Promise<std::shared_ptr<ArrayBuffer>>> HybridPeripheral::read(const std::string& service,
...
std::shared_ptr<Promise<void>> HybridPeripheral::write_request(const std::string& service,
                                                               const std::string& characteristic,
                                                               const std::shared_ptr<ArrayBuffer>& data) {
    SimpleBLE::ByteArray bytes = fromArrayBuffer(data);
    return Promise<void>::async(
        [this, service, characteristic, bytes]() { _peripheral.write_request(service, characteristic, bytes); });
}

void HybridPeripheral::write_command(const std::string& service, const std::string& characteristic,
                                     const std::shared_ptr<ArrayBuffer>& data) {
    SimpleBLE::ByteArray bytes = fromArrayBuffer(data);
    _peripheral.write_command(service, characteristic, bytes);
}

std::shared_ptr<Promise<void>> HybridPeripheral::notify(
...
std::shared_ptr<Promise<void>> HybridPeripheral::indicate(
    const std::string& service, const std::string& characteristic,
    const std::function<void(const std::shared_ptr<ArrayBuffer>&)>& callback) {
...
std::shared_ptr<Promise<void>> HybridPeripheral::unsubscribe(const std::string& service,
                                                             const std::string& characteristic) {
    auto key = std::make_pair(service, characteristic);
    _notifyCallbacks.erase(key);
    return Promise<void>::async(
        [this, service, characteristic]() { _peripheral.unsubscribe(service, characteristic); });
}

std::shared_ptr<Promise<std::shared_ptr<ArrayBuffer>>> HybridPeripheral::read_descriptor(
    const std::string& service, const std::string& characteristic, const std::string& descriptor) {
...
std::shared_ptr<Promise<void>> HybridPeripheral::write_descriptor(const std::string& service,
                                                                  const std::string& characteristic,
...
SimpleBLE::ByteArray HybridPeripheral::fromArrayBuffer(const std::shared_ptr<ArrayBuffer>& buffer) {
    return SimpleBLE::ByteArray(static_cast<const uint8_t*>(buffer->data()), buffer->size());
}
