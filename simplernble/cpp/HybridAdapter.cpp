...
std::shared_ptr<Promise<bool>> HybridAdapter::bluetooth_enabled() {
    return Promise<bool>::async([]() -> bool { return SimpleBLE::Adapter::bluetooth_enabled(); });
}
...
bool HybridAdapter::initialized() { return _adapter.initialized(); }

std::string HybridAdapter::identifier() { return _adapter.identifier(); }

std::string HybridAdapter::address() { return _adapter.address(); }

bool HybridAdapter::is_powered() { return _adapter.is_powered(); }

void HybridAdapter::scan_start() { _adapter.scan_start(); }

void HybridAdapter::scan_stop() { _adapter.scan_stop(); }

std::shared_ptr<Promise<void>> HybridAdapter::scan_for(double timeout_ms) {
    int timeout = static_cast<int>(timeout_ms);
    return Promise<void>::async([this, timeout]() { _adapter.scan_for(timeout); });
}

bool HybridAdapter::scan_is_active() { return _adapter.scan_is_active(); }
