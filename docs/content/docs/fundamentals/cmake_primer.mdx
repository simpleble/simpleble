...
## Consuming CMake Projects from Xcode

When using a CMake-based library from Xcode, create a small CMake consumer project instead of manually adding the built library file to a native Xcode project. Find the installed package and link against its exported imported target, such as `simpleble::simpleble`, in your `CMakeLists.txt`:
...
target_link_libraries(MyApp PRIVATE simpleble::simpleble)
```

Then generate an Xcode project from that consumer project:
