...
SimpleBLE can fill either role. Central APIs (`SimpleBLE::Adapter` scan and `SimpleBLE::Peripheral`) are available on every supported platform. Hosting a local peripheral (`SimpleBLE::Local::Peripheral`) is currently Linux and C++ only; see [Advertise, Serve, Publish](../simpleble/peripheral/advertise-serve-publish).
