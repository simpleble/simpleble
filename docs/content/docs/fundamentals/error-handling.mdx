...
| `OperationNotSupported` | The characteristic doesn't have the required capability, or the backend doesn't support the operation | e.g. calling `read()` on a write-only characteristic, or `create_local_peripheral()` outside Linux and macOS |
