...
- (SimpleBLE) Fixed macOS notification callbacks to snapshot payload data before invoking the user callback.
- (SimpleBLE) Fixed concurrent access to macOS peripheral advertising data while scan updates are being processed.
- (SimpleBLE) Fixed concurrent access to macOS adapter peripheral maps while scan and connection events are processed.
