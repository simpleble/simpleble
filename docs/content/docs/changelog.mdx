...
- (Linux) Switched the default BlueZ backend from the legacy implementation to the new SimpleBluez backend.
- (Linux) Start the new BlueZ backend dispatch loop before registering the pairing agent.
...
- (Linux) Fixed the new BlueZ backend to wait for locally-dispatched service resolution before reporting a peripheral as connected.
- (Linux) Fixed the new BlueZ backend to avoid timing out connections when services were already locally resolved.
- (Linux) Fixed `Adapter::power_on` and `Adapter::power_off` in the new BlueZ backend.
- (Linux) Fixed BlueZ backend selection to avoid constructing both the new and legacy backends.
- (Linux) Fixed concurrent access to scan result maps in the new BlueZ backend.
- (Linux) Fixed random address types being reported as unspecified in the new BlueZ backend.
