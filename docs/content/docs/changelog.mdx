...
- (SimplePyBLE) Added `simplepyble.aio` module providing an `asyncio`-compatible API for all library classes (`Adapter`, `Peripheral`, etc.).
- (SimplePyBLE) Added automatic callback deregistration and scanning cleanup on process exit for `simplepyble.aio`.
- (SimplePyBLE) Added `async with` context manager support to `aio.Adapter` and `aio.Peripheral` for idiomatic resource management.
