...
- (Java) Added `CompletableFuture` wrappers for blocking operations to allow clean asynchronous programming without blocking threads.

**Changed**

- (Java) JNI API calls are now safely wrapped to catch C++ exceptions and bubble them up as Java `RuntimeException` instead of crashing the JVM.
- (Java) Updated `EventListener` interfaces in `Adapter` and `Peripheral` to provide default empty implementations.
...
- (Rust) Fixed linker errors on some 64-bit Linux systems. **(Thanks, davidedellagiustina!)**
- (Java) Fixed missing macOS x64 `.dylib` artifact in the release package due to an omitted CI architecture target.
- (Java) Fixed JNI symbol resolution errors (`UnsatisfiedLinkError`) on `Adapter.isBluetoothEnabled()` and `isPaired` flags.
- (Java) Fixed `jvm.dll` initialization conflicts on Windows that prevented the native bindings from loading smoothly.
