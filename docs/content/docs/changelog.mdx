...
Windows builds using MSVC now require a C++20-capable toolchain, such as Visual Studio 2019 16.11 or newer.
This keeps the WinRT backend on standard coroutine support instead of the deprecated `<experimental/coroutine>` path.
...
- (Windows) Build the SimpleBLE target as C++20 when using MSVC to avoid deprecated C++/WinRT coroutine headers.
