...
Android's public legacy advertising API can include only the system Bluetooth name. On Android, omit `local_name` or set it to `adapter.identifier()`; `start()` rejects an arbitrary name with a clear error instead of silently advertising a different value.
...
- [Platform notes](../platform_notes) for platform-specific behavior
