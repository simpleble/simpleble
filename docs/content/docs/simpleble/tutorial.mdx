...
With Bluetooth access confirmed, we need to discover the adapters available on the machine. To keep things simple, we will just use the first adapter that we find:
...
2. `SimpleBLE::Adapter::get_adapters()` returns the Bluetooth adapters available on the system. To keep things simple, the program uses the first one it finds.
