...
Set configuration values before the first adapter call. For example, Unity or other P/Invoke users on Android can request high connection priority with:

```c
#include <simplecble/simplecble.h>

int main(void) {
    simpleble_config_set_android_connection_priority(1);

    size_t adapter_count = simpleble_adapter_get_count();
    /* Continue with adapter selection and scanning. */
}
```
