...
2. Review the version number in the following files, make sure they match:
   ...
   - `docs/content/docs/changelog.mdx` (Add the release date)
   - `simpleaible/pyproject.toml` (Remove the -dev1 suffix)

3. Commit the changes to the repository.

4. Push the release commit to the remote repository without a tag.

5. Wait for the `CI Main` workflow triggered by the branch push to finish successfully.

6. Tag the commit with the new version number using the `vX.Y.Z` format.

7. Push the tag to the remote repository.

8. Create and publish a new release on GitHub for that tag.

9. Wait for the release-triggered `CI Main` workflow to finish. This workflow uploads the release artifacts and publishes the packages.

10. Advance the version number in the following files:
...
11. Run `cargo build` to ensure that `Cargo.toml` is correct.

12. Run `minion deploy` to deploy the documentation.

13. TODO: SOCIAL MEDIA
