...
7. Wait for the branch-triggered `CI Main` workflow to finish successfully. A `main` push publishes development packages: `X.Y.Z-devN` to crates.io and Maven Central, and `X.Y.Z.devN` to PyPI.

   Confirm that SimpleRsBLE, SimplePyBLE, SimpleAIBLE, SimpleJavaBLE, SimpleDroidBridge, and SimpleDroidBLE all use the expected development version. If a fix is required, push the fix and wait for its workflow to finish before continuing.
...
12. Verify the published release:
    ...
    - SimpleJavaBLE `X.Y.Z` is available on Maven Central.
    - SimpleDroidBLE and SimpleDroidBridge `X.Y.Z` are available on Maven Central.
...
16. Wait for the branch-triggered `CI Main` workflow to finish successfully and confirm that the new development packages were published to crates.io, PyPI, and Maven Central with the expected `-devN`/`.devN` versions.

17. From the documentation root, run `berth app deploy` to deploy the documentation.
