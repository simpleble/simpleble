...
SimplePyBLE also ships an MCP server powered by FastMCP for local tool access from MCP-capable clients. Install the optional dependencies:
...
Manual smoke test: use an MCP client to call `adapters_list` and `scan_run` once the server is running.