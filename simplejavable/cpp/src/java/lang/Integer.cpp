...
int Integer<RefType>::int_value() const {
    ...
    JNIEnv* env = SimpleJNI::VM::env();
    return env->CallIntMethod(_obj.get(), _method_int_value);
}

template <template <typename> class RefType>
Integer<SimpleJNI::LocalRef> Integer<RefType>::value_of(int value) {
    if (!_cls.get()) {
        throw std::runtime_error("Integer JNI resources not preloaded");
    }
    JNIEnv* env = SimpleJNI::VM::env();
    jobject result = env->CallStaticObjectMethod(_cls.get(), _method_value_of, value);
    return Integer<SimpleJNI::LocalRef>(result);
}
